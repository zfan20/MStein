...
from data_augmentation import Crop, Mask, Reorder, Substitute, Insert, Random, CombinatorialEnumerate, RandomModelAug, Identity, RandomCL4Rec
from utils import neg_sample, nCr
import copy

class RecWithCL4RecContrastiveLearningDataset(Dataset):
    def __init__(self, args, user_seq, test_neg_items=None, data_type='train'):
        ...
        self.augmentations = {'crop': Crop(tao=args.tao),
                              ...
                              'random': RandomCL4Rec(tao=args.tao, gamma=args.gamma, beta=args.beta)
                              }
...
class RecWithModeAugContrastiveLearningDataset(Dataset):
    def __init__(self, args, user_seq, test_neg_items=None, data_type='train'):
        self.args = args
        self.user_seq = user_seq
        self.test_neg_items = test_neg_items
        ...
        self.augmentations = {'crop': Crop(tao=args.tao),
                              ...
                              'reorder': Reorder(beta=args.beta),
                              'identity': Identity(),
                              'random' : RandomModelAug(tao=args.tao,gamma=args.gamma, beta=args.beta),
                            }
        if self.args.base_augment_type not in self.augmentations:
            raise ValueError(f"augmentation type: '{self.args.base_augment_type}' is invalided")
        print(f"Creating Contrastive Learning Dataset using '{self.args.base_augment_type}' data augmentation")
